...
export type ConnectionStatus = 'connected' | 'disconnected' | 'polling' | 'loading';
...
export async function fetchTickerREST(symbol: string): Promise<TickerData | null> {
  try {
    const res = await fetch(`${BINANCE_SPOT_REST_BASE}/api/v3/ticker/24hr?symbol=${symbol}`);
    ...
    return {
      ...
      openPrice: parseFloat(d.openPrice) || 0,
...
export async function fetchBinancePrices(symbols: string[]): Promise<Map<string, number>> {
  ...
  const results = await Promise.allSettled(symbols.map((s) => fetchTickerREST(s)));
  results.forEach((r, i) => {
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          const ticker: TickerData = {
            symbol: d.s,
            lastPrice: parseFloat(d.c) || 0,
            priceChangePercent: parseFloat(d.P) || 0,
          ...
          this.tickerCallbacks.forEach((cb) => cb(d.s, ticker));
        } catch {
          // ignore parse errors
        }
      };
