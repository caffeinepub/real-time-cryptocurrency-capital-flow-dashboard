import { X, TrendingUp, TrendingDown, Activity, Layers, Target, BarChart3 } from 'lucide-react';
...
export default function BubbleDetailPanel({ asset, onClose }: BubbleDetailPanelProps) {
  ...
  const { data: chartData, isLoading: chartLoading, isError: chartError } = useTrendChartData(asset.symbol);
      ...
      >
        <Card className="bg-card/95 backdrop-blur-xl border-border/50 shadow-2xl">
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between">
              <div className={`flex items-center gap-3 transition-all duration-500 delay-100 ${
                ...
                {getTrendIcon()}
                <div>
                  <CardTitle className="text-2xl font-bold">{asset.symbol.replace('USDT', '')}</CardTitle>
          ...
          <CardContent className="space-y-4">
            {/* Price and Change */}
            <div className={`grid grid-cols-2 gap-4 transition-all duration-500 delay-150 ${
              isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
            }`}>
              <div className="p-3 rounded-lg bg-accent/50 border border-border/30">
                <p className="text-xs text-muted-foreground mb-1 font-semibold">Preço Atual</p>
                <p className="text-lg font-bold">
                  {detailData?.livePrice ? formatCurrency(detailData.livePrice) : formatCurrency(asset.price)}
                </p>
              </div>
              <div className="p-3 rounded-lg bg-accent/50 border border-border/30">
                <p className="text-xs text-muted-foreground mb-1 font-semibold">Variação 24h</p>
            ...
            {/* Volume */}
            {detailData?.volume && (
              <div className={`p-3 rounded-lg bg-accent/50 border border-border/30 transition-all duration-500 delay-200 ${
                isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
              }`}>
                <p className="text-xs text-muted-foreground mb-1 font-semibold">Volume 24h</p>
                <p className="text-base font-bold">{formatCurrency(detailData.volume)}</p>
              </div>
            )}
            ...
            <div className={`transition-all duration-500 delay-250 ${
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-sm font-semibold">Gráfico de Tendência (4h)</p>
              </div>
              ...
              {chartLoading ? (
                ...
                <div className="w-full h-64 flex items-center justify-center bg-destructive/10 rounded-lg border border-destructive/30">
                  <div className="text-center space-y-2 px-4">
                    <p className="text-sm text-destructive font-medium">Erro ao carregar gráfico</p>
                    <p className="text-xs text-muted-foreground">Dados históricos indisponíveis</p>
                  </div>
                </div>
            ...
            <div className={`transition-all duration-500 delay-300 ${
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <Badge variant="outline" className={`${getTrendColor()} font-bold text-base px-3 py-1`}>
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                {/* Flow Intensity */}
                <div className={`p-4 rounded-lg bg-neon-cyan/10 border border-neon-cyan/30 transition-all duration-500 delay-350 hover:bg-neon-cyan/15 hover:border-neon-cyan/50 hover:shadow-lg hover:shadow-neon-cyan/20 ${
                  isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
                }`}>
                  <div className="flex items-center gap-2 mb-2">
                    <Activity className="w-5 h-5 text-neon-cyan" />
                    <p className="text-sm font-semibold">Intensidade de Fluxo</p>
                  </div>
                  <p className="text-3xl font-bold text-neon-cyan mb-1">{formatNumber(asset.flowIntensity)}%</p>
                  {detailData?.capitalFlow && (
                    <div className="mt-2 text-xs text-muted-foreground space-y-1">
                      <p className="font-medium">PnL Ratio: <span className="text-foreground">{formatNumber(detailData.capitalFlow.pnlRatio)}</span></p>
                      <p className="font-medium">Impacto: <span className="text-foreground">{formatNumber(detailData.capitalFlow.marketImpact)}</span></p>
                    </div>
                  )}
                </div>

                {/* Confluence Intensity */}
                <div className={`p-4 rounded-lg bg-neon-green/10 border border-neon-green/30 transition-all duration-500 delay-400 hover:bg-neon-green/15 hover:border-neon-green/50 hover:shadow-lg hover:shadow-neon-green/20 ${
                  isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
                }`}>
                  <div className="flex items-center gap-2 mb-2">
                    <Layers className="w-5 h-5 text-neon-green" />
                    <p className="text-sm font-semibold">Intensidade de Confluência</p>
                  </div>
                  <p className="text-3xl font-bold text-neon-green mb-1">{formatNumber(asset.confluenceIntensity)}%</p>
                  {detailData?.confluenceZone && (
                    <div className="mt-2 text-xs text-muted-foreground">
                      <p className="font-medium">Indicadores: <span className="text-foreground">{detailData.confluenceZone.indicators.join(', ')}</span></p>
                    </div>
                  )}
                </div>

                {/* Confidence Level */}
                <div className={`p-4 rounded-lg bg-neon-purple/10 border border-neon-purple/30 transition-all duration-500 delay-450 hover:bg-neon-purple/15 hover:border-neon-purple/50 hover:shadow-lg hover:shadow-neon-purple/20 ${
                  isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
                }`}>
                  <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-3xl font-bold text-neon-purple mb-1">{formatNumber(asset.confidenceLevel)}%</p>
                  <p className="text-xs text-muted-foreground mt-2">
                    Tendência: <span className={`${getTrendColor()} font-bold text-sm`}>{asset.trend}</span>
                  </p>
                </div>
              </div>
            )}
