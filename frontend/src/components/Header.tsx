...
import { BarChart2, Globe, TrendingUp, Layers, Activity, Wifi, WifiOff, Loader2 } from 'lucide-react';
...
const TABS: { id: TabId; label: string; icon: React.ReactNode }[] = [
  ...
  { id: 'confluence', label: 'Confluências', icon: <Layers className="w-4 h-4" /> },
  { id: 'futures', label: 'Order Flow', icon: <Activity className="w-4 h-4" /> },
];
...
export default function Header({ activeTab, onTabChange, connectionStatus }: HeaderProps) {
  return (
      ...
      <div className="flex items-center justify-between px-3 sm:px-4 h-14">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <span className="font-bold text-sm text-foreground tracking-tight leading-none block">CryptoFlow</span>
            <span className="text-[10px] text-muted-foreground leading-none">Intelligence</span>
          </div>
        </div>
