...
export function useOpenFuturesPositions(enablePolling = false) {
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (errorMessage.includes("No Binance credentials")) {
          throw new Error(
            "Please configure your Binance API credentials to view positions.",
          );
        }
        ...
        if (errorMessage.includes("Binance") || errorMessage.includes("API")) {
          throw new Error(
            "Failed to fetch positions from Binance. Please check your credentials and try again.",
          );
        }
