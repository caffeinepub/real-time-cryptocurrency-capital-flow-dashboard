...
export default function ConfluenceZones() {
      ...
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground">
            Carregando zonas de confluência...
          </p>
        </div>
      </div>
    );
  ...
  const sortedZones = displayZones
    ? [...displayZones].sort((a, b) => b.intensity - a.intensity)
    : [];
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Detecção de sinais sobrepostos e indicadores convergentes
          </p>
        </div>
      ...
      {!isLive && !hasData && (
        <Alert className="border-neon-pink/50 bg-neon-pink/10">
          ...
          <AlertDescription className="text-neon-pink">
            Não foi possível conectar à API Binance. Exibindo zonas em cache.
            Tentando reconectar...
          </AlertDescription>
        </Alert>
      )}
      ...
      {sortedZones.length === 0 ? (
        <div className="text-center py-16 bg-card/50 rounded-xl border border-border/50">
          ...
          <p className="text-muted-foreground">
            Nenhuma zona de confluência detectada ainda
          </p>
          <p className="text-sm text-muted-foreground/70 mt-2">
            {isLive
              ? "As zonas aparecerão quando múltiplos sinais se alinharem"
        ...
        <>
          <div className="grid grid-cols-1 gap-6">
            {sortedZones.map((zone, index) => (
              <ConfluenceZoneCard
                key={String(zone.timestamp) || index}
                zone={zone}
                index={index}
              />
            ))}
          ...
          <div className="mt-8 bg-card/30 backdrop-blur-sm rounded-xl border border-border/50 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 rounded-full bg-neon-green shadow-neon-green-sm" />
                <div>
              ...
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 rounded-full bg-neon-blue shadow-neon-blue-sm" />
                <div>
                  <p className="text-sm font-medium text-foreground">
                    Intensidade Média
                  </p>
              ...
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 rounded-full bg-neon-cyan shadow-neon-cyan-sm" />
                <div>
                  <p className="text-sm font-medium text-foreground">
                    Intensidade Baixa
                  </p>
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-bold text-foreground">
              Zona de Confluência #{index + 1}
            </h3>
            <p className="text-sm text-muted-foreground">
              {zone.indicators.length} indicadores sobrepostos
            </p>
          </div>
        </div>
        <div
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between text-sm mb-3">
          ...
          <span className={`font-bold ${styles.color} text-lg`}>
            {formatPercentage(zone.intensity)}
          </span>
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {zone.indicators.map((indicator, _idx) => (
            <div
              key={indicator}
