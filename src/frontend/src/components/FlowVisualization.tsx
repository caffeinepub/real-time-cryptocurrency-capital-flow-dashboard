...
export default function FlowVisualization() {
  ...
  const [interpretationSummary, setInterpretationSummary] =
    useState<string>("");
  ...
  useEffect(() => {
    if (animatedFlows.length > 0) {
      ...
      const avgAltcoinIntensity =
        altcoinFlows.length > 0
          ? totalAltcoinIntensity / altcoinFlows.length
          : 0;
      ...
      if (btcFlow && btcFlow.flowIntensity > 0.6) {
        ...
        btcFlow &&
        ethFlow &&
        btcFlow.flowIntensity > 0.4 &&
        ethFlow.flowIntensity > 0.4
      ) {
      ...
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground">
            Carregando dados de fluxo de capital...
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Movimentação de capital USD para criptomoedas em tempo real
          </p>
        </div>
      ...
      {!isLive && !hasData && (
        <Alert className="border-neon-pink/50 bg-neon-pink/10">
          ...
          <AlertDescription className="text-neon-pink">
            Não foi possível conectar à API Binance. Exibindo dados em cache.
            Tentando reconectar...
          </AlertDescription>
        </Alert>
      )}
      ...
      {flowsWithPercentages.length === 0 ? (
        <div className="text-center py-16 bg-card/50 rounded-xl border border-border/50">
          ...
          <p className="text-muted-foreground">
            Nenhum dado de fluxo de capital disponível ainda
          </p>
          <p className="text-sm text-muted-foreground/70 mt-2">
            {isLive
              ? "Os dados de fluxo aparecerão aqui em tempo real"
        ...
        <StatCard
          ...
          value={
            animatedFlows.length > 0
              ? formatNumber(
                  animatedFlows.reduce((sum, f) => sum + f.flowIntensity, 0) /
                    animatedFlows.length,
                )
              : formatNumber(0)
          }
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-right">
          <div className="text-2xl font-bold text-foreground animate-pulse-slow">
            {formatCurrency(flow.amount)}
          </div>
          {flow.percentage !== undefined && (
            <div className="text-sm text-muted-foreground">
              {formatNumber(flow.percentage)}% do total
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div>
          <div className="flex items-center justify-between text-sm mb-2">
            ...
            <span className={`font-medium ${styles.color} animate-pulse-slow`}>
              {formatPercentage(flow.flowIntensity)}
            </span>
          </div>
...
function StatCard({ title, value, icon, color }: StatCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`text-3xl font-bold ${classes.text} animate-pulse-slow`}>
        {value}
      </div>
    </div>
  );
