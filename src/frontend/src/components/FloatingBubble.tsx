...
import {
  ...
  calculateOscillationAmplitude,
  calculatePhaseOffset,
} from "../lib/bubbleLayout";
import {
  calculateGlowIntensity,
  getEmissiveColor,
  mapConvergenceToSize,
  mapTrendToColor,
} from "../lib/bubbleVisualMapping";
...
export default function FloatingBubble({
  ...
  useFrame((state, delta) => {
    if (meshRef.current) {
      ...
      const floatY =
        Math.sin(time * animationParams.speed + animationParams.phaseOffset) *
        animationParams.amplitude;
      const floatX =
        Math.cos(
          time * animationParams.speed * 0.7 +
            animationParams.phaseOffset * 1.3,
        ) *
        (animationParams.amplitude * 0.5);
      const floatZ =
        Math.sin(
          time * animationParams.speed * 0.5 +
            animationParams.phaseOffset * 0.8,
        ) *
        (animationParams.amplitude * 0.3);
      ...
      const pulseScale =
        1 +
        Math.sin(
          time * animationParams.speed * 2 + animationParams.phaseOffset,
        ) *
          0.02;
