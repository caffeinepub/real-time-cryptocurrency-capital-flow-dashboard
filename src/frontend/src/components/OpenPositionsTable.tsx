...
export default function OpenPositionsTable({
    ...
    return (
      <Alert className="border-neon-cyan/30 bg-neon-cyan/5">
        <AlertDescription className="text-sm text-muted-foreground">
          No open positions found. Your positions will appear here once you open
          trades on Binance Futures.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  const positionsWithWarnings = positions.filter((pos) =>
    generatePositionInsights(pos).some(
      ...
      {positionsWithWarnings > 0 && (
        <Alert className="border-neon-yellow/30 bg-neon-yellow/5">
          ...
          <AlertDescription className="text-sm text-neon-yellow">
            {positionsWithWarnings} position
            {positionsWithWarnings > 1 ? "s" : ""} with risk alerts detected.
          </AlertDescription>
        </Alert>
      )}
        ...
        {positions.map((position) => {
          ...
          return (
            <Card
              ...
              className="terminal-panel border-neon-green/30 bg-card/50"
            >
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-sm font-semibold ${getPositionSideColor(position.positionSide)}`}
                    >
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-foreground">
                      {formatSize(Math.abs(position.positionAmt))}
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-foreground">
                      ${formatLiquidationPrice(position)}
                    </p>
                  </div>
                </div>
                ...
                <div className="pt-2 border-t border-border/50">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-lg font-bold ${getPnLColor(position.pnl)}`}
                    >
